...
def record_tool_result(ctx: "AuthContext", result: dict[str, Any]) -> None:
    ...
    if ctx.role == 'merchant' and ctx.store_id is not None: span.set_attribute("cartwheel.store_id", int(ctx.store_id))
...
def _set_permission_denied_attributes(
    ...
    """
    pd_att = result.get("error") == "permission_denied"
    span.set_attribute("cartwheel.permission_denied", pd_att)
    if pd_att:
